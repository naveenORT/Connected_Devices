    ...
    * This module facilitates in establishing smtp server by invoking smtpclientconnector function. It uses publishMessage function
    ...
    def sendNotification(self):  # Function to Publish Alerts As Mail
        ...
        if(data_object.getcurvalue() < self.temp_set_point):
            logging.info("Current temperature is " + float(self.nominal_temp) - data_object.getcurvalue() + "C lesser than nomainal temp ")
        if(data_object.getcurvalue() > self.temp_set_point):
            logging.info("Current temperature is " + data_object.getcurvalue() - float(self.nominal_temp) + "C greater than nomainal temp ")
